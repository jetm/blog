...
Changing hardware never means starting over, either. I don't remember the last time I actually installed a distro from scratch. When I move to new hardware, I either rsync the running system onto the new drive, or move the same drive into the new machine and boot an Arch Linux live ISO just long enough to reinstall the bootloader for the new disk controller. Past that it's a hostname change and, if I'm crossing from AMD to Intel or back, swapping the CPU microcode package and whatever vendor-specific driver packages no longer apply. Everything else, the shell config, the package set, the dotfiles, comes along because none of it was ever tied to the installer that put it there in the first place.
